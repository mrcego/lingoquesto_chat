...
import type { VoiceMessage, User, OnlineUser } from '@/types/chat';
...
export const useChatStore = defineStore('chat', () => {
   ...
   */
  const onlineUsers = ref<Array<OnlineUser>>([]);
   ...
   */
  const setMessages = (newMessages: Array<VoiceMessage>) => {
   ...
   */
  const setOnlineUsers = (users: Array<OnlineUser>) => {
    console.log(
      '📊 Setting online users in store:',
  ...
  const isUserOnline = (nickname: string): boolean => {
    const foundUser = onlineUsers.value.find((user) => user.nickname === nickname);
